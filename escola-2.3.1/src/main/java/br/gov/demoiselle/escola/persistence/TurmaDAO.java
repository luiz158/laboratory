...
public class TurmaDAO extends JPACrud<Turma, Long>{
	...
	public List<Turma> buscarPorProfessor(Professor professor){
		Query query = createQuery("from Turma as turma where turma.professor.id = " + professor.getId());
		return query.getResultList();
	}

	public List<Turma> buscarPorDisciplina(Disciplina disciplina) {
		Query query = createQuery("from Turma as turma where turma.disciplina.id = " + disciplina.getId());
		return query.getResultList();
	}
