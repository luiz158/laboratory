...
import br.gov.frameworkdemoiselle.snmp.security.AccessEntry;
import br.gov.frameworkdemoiselle.snmp.security.GroupEntry;
import br.gov.frameworkdemoiselle.snmp.security.SNMPSecurityParser;
import br.gov.frameworkdemoiselle.snmp.security.UserEntry;
import br.gov.frameworkdemoiselle.snmp.security.ViewTree;
import br.gov.frameworkdemoiselle.snmp.security.ViewTreeFamily;
...
public class SNMPAgent extends BaseAgent implements Runnable {
	...
	protected void addNotificationTargets(SnmpTargetMIB targetMIB, SnmpNotificationMIB notificationMIB) {
		...
		targetMIB.addTargetAddress(new OctetString("notification"), TransportDomains.transportDomainUdpIpv4,
				new OctetString(new UdpAddress("127.0.0.1/162").getValue()), 200, 1, new OctetString("notify"),
				new OctetString("v2c"), StorageType.permanent);
		targetMIB.addTargetParams(new OctetString("v2c"), MessageProcessingModel.MPv2c,
				SecurityModel.SECURITY_MODEL_SNMPv2c, new OctetString("public"), SecurityLevel.NOAUTH_NOPRIV,
	...
	protected void addUsmUser(USM usm) {
		...
		for (UsmUser user : users) {
			logger.debug(bundle.getString("agent-snmp-adding-user", user.getSecurityName(),
					user.getAuthenticationProtocol(), user.getPrivacyProtocol()));
	...
	protected void addViews(VacmMIB vacm) {
		...
		for (ViewTreeFamily vtf : views) {
			...
			for (ViewTree view : vtf.getIncludes()) {
				logger.debug(bundle.getString("agent-snmp-including-subtree", view.getSubtree(), view.getMask(),
						vtf.getViewName()));
				vacm.addViewTreeFamily(vtf.getViewName(), view.getSubtree(), view.getMask(), VacmMIB.vacmViewIncluded,
						StorageType.nonVolatile);
			}

			// excludes
			for (ViewTree view : vtf.getExcludes()) {
				logger.debug(bundle.getString("agent-snmp-excluding-subtree", view.getSubtree(), view.getMask(),
						vtf.getViewName()));
				vacm.addViewTreeFamily(vtf.getViewName(), view.getSubtree(), view.getMask(), VacmMIB.vacmViewExcluded,
						StorageType.nonVolatile);
			}
		}
		...
		for (GroupEntry group : groups) {
			...
			for (UserEntry user : group.getUsers()) {
				...
				logger.debug(bundle.getString("agent-snmp-including-user-group", securityName, groupName,
						user.getModel()));
			...
			if (group.getAccess() != null) {
				...
				logger.debug(bundle.getString("agent-snmp-granting-access-views", access.getReadView(),
						access.getWriteView(), access.getNotifyView(), groupName, access.getModel(), access.getLevel(),
						access.getMatch()));

				vacm.addAccess(groupName, access.getContext(), access.getModel().getId(), access.getLevel().getId(),
						access.getMatch().getId(), access.getReadView(), access.getWriteView(), access.getNotifyView(),
						StorageType.nonVolatile);
			}
		}
	}
	...
	protected void registerSnmpMIBs() {
		...
		// Clientes vão registrar os MIBs

		/*
		 * logger.debug(bundle.getString("agent-snmp-registering-mibs")); if (mibs != null) { for (Class<?> clazz :
		 * mibs) { try { manager.registerMIB(clazz); } catch (DuplicateRegistrationException e) {
	 ...
	 */
	public void registerManagedObjects(ManagedClassMonitor monitor) {
		for (MOScalar scalar : monitor.getMOScalarList()){
			try {
				server.register(scalar, null);
			} catch (DuplicateRegistrationException ex) {
				logger.warn(bundle.getString("agent-snmp-duplicated-registration-error", monitor.getManagedClass()));
			}
		}
	}
	
	public void unregisterManagedObjects(ManagedClassMonitor monitor) {
		logger.info(bundle.getString("agent-snmp-unregistering-mib",monitor.getManagedClass()));
		for (MOScalar scalar : monitor.getMOScalarList()){
			server.unregister(scalar, null);
		}
	}
	...
	public void startup() {
		try {
			...
			throw new DemoiselleException(bundle.getString("agent-snmp-startup-error", e));
		}
