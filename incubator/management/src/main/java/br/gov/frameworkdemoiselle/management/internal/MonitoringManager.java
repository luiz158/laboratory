...
/**
 * A manager that helps implementators of the management framework to obtain a list of managed classes, set or obtain
 * values from them or invoke operations over them.
 * 
 * @author serpro
 */
@ApplicationScoped
public class MonitoringManager {
	...
	/**
	 * @return A list all managed types, classes annotated with {@link Managed}. The returned list is a shallow copy of
	 *         the internal list, so you are free to modify it.
	 */
	 ...
	 * 
	 * @param managedType
	 *            A type annotated with {@link Managed}. This method will create an (or obtain an already created)
	 *            instance of this type and invoke the operation over it.
	 * @param actionName
	 ...
	 */
	public Object invoke(Class<?> managedType, String actionName, Object[] params) {
		...
		if (index > -1) {
			...
			if (method != null) {
				try {
					logger.debug(bundle.getString("management-debug-invoking-operation"));
	...
	public Object getAttribute(Class<?> managedType, String attribute) {
		...
		if (index > -1) {
			...
			if (getterMethod != null) {
				logger.debug(bundle.getString("management-debug-acessing-property", getterMethod.getName(), type
				...
				throw new DemoiselleException(bundle.getString("management-invoke-error", attribute));
			}
		} else {
			throw new DemoiselleException(bundle.getString("management-type-not-found"));
		}
	...
	public void setAttribute(Class<?> managedType, String attribute,Object newValue) {
		...
		if (index > -1) {
			...
			if (method != null) {
				logger.debug(bundle.getString("management-debug-setting-property", method.getName(),
	...
	private void activateContexts(Class<?> managedType) {
		logger.debug("management-debug-starting-custom-context",ManagedContext.class.getCanonicalName(),managedType.getCanonicalName());
		Contexts.activate(ManagedContext.class);
	}

	private void deactivateContexts(Class<?> managedType) {
		logger.debug("management-debug-stoping-custom-context",ManagedContext.class.getCanonicalName(),managedType.getCanonicalName());
